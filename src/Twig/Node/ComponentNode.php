        ...
        if ($data !== NULL) {
            ...
            foreach ($data->getKeyValuePairs() as $pair) {
                if ($pair['key']->getAttribute('value') === $dataKey) {
                    ...
                    if(get_class($pair['value']) === 'Twig_Node_Expression_Array') {

                        foreach($pair['value']->getKeyValuePairs() as $pair_value) {

                            $keyValuePairs[] = $pair_value;
                        }
            ...
            foreach ($keyValuePairs as $pair) {
                ...
                if ($compileStringPrefix !== NULL) {
                    ...
                    $string = ($pair['value']->hasAttribute('value')) ? $pair['value']->getAttribute('value') : $pair['value']->getAttribute('name');
