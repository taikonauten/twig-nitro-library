...
class ComponentNode extends Twig_Node implements Twig_NodeOutputInterface {
  ...
  public function compile(Twig_Compiler $compiler) {
    ...
    $compiler->raw('->display(array_merge($tContext, [');
      ...
      if ($data instanceof Twig_Node_Expression_Array) {
        foreach ($data->getKeyValuePairs() as $pair) {
          if ($pair['key']->getAttribute('value') === $dataKey) {
            ...
            if ($pair['value'] instanceof Twig_Node_Expression_Array) {
              foreach ($pair['value']->getKeyValuePairs() as $subPairValue) {
                $keyValuePairs[] = $subPairValue;
              }
            }
            else {
              $keyValuePairs[] = $pair;
   ...
   *   The key value pairs to compile.
   * @param string $glue
   *   What to use as glue between compiled items.
   * @param string|null $compileStringPrefix
   *   If given, this string is prepended to each compiled item.
   ...
   *   The Twig compiler.
   */
  private function compileAndImplodeKeyValuePairs(
    Twig_Compiler $compiler,
    array $keyValuePairs,
    string $glue,
    string $compileStringPrefix = NULL,
    ...
    foreach ($keyValuePairs as $pair) {
      ...
      // Is not last item.
      if ($dataPairIndex !== count($keyValuePairs) - 1) {
        $compiler->raw($glue);
      }
